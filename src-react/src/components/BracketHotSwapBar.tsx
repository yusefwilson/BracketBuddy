...
export default function BracketHotSwapBar({ tournament, currentBracketIndex, onBracketChange }: BracketHotSwapBarProps) {
    return (
        <div className="w-full bg-slate-600 p-2 rounded-md shadow-inner overflow-x-auto flex gap-2">
            {tournament.brackets.map((bracket, index) => (
                <button
                    key={bracket.id}
                    onClick={() => onBracketChange(index)}
                    className={`flex-shrink-0 px-4 py-2 rounded-md font-semibold transition-colors duration-200
