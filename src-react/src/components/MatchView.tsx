...
export default function MatchView({ match, updateMatch, x, y, currentMatchId }:
  ...
  let colorStyle = 'bg-blue-400';

  if (highlighted) {
    colorStyle = 'bg-yellow-500';
  } else if (stale) {
    colorStyle = 'bg-gray-500';
  }
  ...
  return (
    <div className='absolute' style={{
      ...
      {/* Match container */}
      <div className={'rounded-md bg-transparent p-2 flex flex-row gap-2 '}>
        {/* Match ID vertically centered */}
        <div className='flex items-center'>
          <h3 className='text-center font-bold'>{match.id}.</h3>
        </div>
        <div className='flex flex-col gap-2'>
          <div className={'flex flex-row justify-between items-center p-2 rounded-md w-44 ' + colorStyle} onClick={() => toggleWinner(0)}>
          ...
          <div className={'flex flex-row justify-between items-center p-2 rounded-md w-44 ' + colorStyle} onClick={() => toggleWinner(1)}>
            <div className='text-sm truncate w-0 flex-1'>
